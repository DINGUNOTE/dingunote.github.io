...
`부분 적용 함수(partially applied function)`란 n개의 인자를 받는 함수에 미리 m개의 인자만 넘겨 기억시켰다가, 나중에 (n-m)개의 인자를 넘기면 비로소 원래 함수의 실행 결과를 얻을 수 있게끔 하는 함수다.
  ...
  ```
  ‘비워놓음’을 표시하기 위해 전역객체에 '_'라는 프로퍼티를 준비해서 삭제, 변경 등 접근에 대한 방어를 위해 여러 프로퍼티 속성을 설정했다. _로 비워놓은 공간에 나중에 넘어온 인자들이 차례대로 끼워넣도록 구현했다. 미리 실행할 함수의 모든 인자 갯수에 맞춰 빈 공간을 확보하지 않아도 되고, 실행할 함수 내부 로직에만 문제가 없으면 최종 실행 시 인자 개수가 많든 적든 잘 실행될 것이다.
